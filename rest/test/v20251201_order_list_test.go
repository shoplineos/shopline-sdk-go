...
import (
	...
	order2 "github.com/shoplineos/shopline-sdk-go/rest/v20251201/order"
	"net/http"
	"reflect"
	"runtime"
	"testing"
...
func TestOrderListAll(t *testing.T) {
	...
	cases := []struct {
		name                string
		expectedOrders      []order2.GetOrdersAPIRespOrder
	...
	}{
		{
			...
			expectedOrders: []order2.GetOrdersAPIRespOrder{{Id: "1"}, {Id: "2"}, {Id: "3"}, {Id: "4"}},
			expectedErr:    nil,
		},
		{
			...
			expectedOrders: []order2.GetOrdersAPIRespOrder{{Id: "1"}},
			expectedErr:    nil,
		},
		{
...
func TestOrderListWithPagination(t *testing.T) {
	...
	cases := []struct {
		...
		// Expect empty pagination when there is no link header
		{
			`{"orders": [{"id":"1"},{"id":"2"}]}`,
			"",
			[]order2.GetOrdersAPIRespOrder{{Id: "1"}, {Id: "2"}},
		...
		// Invalid link header responses
		{
			"{}",
			"invalid link",
			[]order2.GetOrdersAPIRespOrder(nil),
			nil,
		...
		// Valid link header responses
		{
			...
			[]order2.GetOrdersAPIRespOrder{{Id: "1"}},
			&client.Pagination{
		...
		},
		{
			...
			[]order2.GetOrdersAPIRespOrder{{Id: "2"}},
			&client.Pagination{
