// Package product generated by SHOPLINE since v20260301
package product

import (
	"github.com/shoplineos/shopline-sdk-go/client"
)

func (e *InventoryHorizontalConnectionEvent) GetSupportedTopic() string {
	return "inventory_levels/connect"
}

type InventoryHorizontalConnectionEvent struct {
	client.WebhookEvent
	/*
	   The ID of the Inventory item.
	   Example: 5703943240925324252
	*/
	InventoryItemId string `json:"inventory_item_id,omitempty" url:"-"`

	/*
	   The ID of location.
	   Example: 5421704248135526901
	*/
	LocationId string `json:"location_id,omitempty" url:"-"`

	/*
	   The actual quantity of products owned by the location.
	   Example: 10
	*/
	Onhand int `json:"onhand,omitempty" url:"-"`

	/*
	   The quantity that has been reserved and is not available for sale.
	   Example: 10
	*/
	Unavailable int `json:"unavailable,omitempty" url:"-"`

	/*
	   Date and time of the last update to the inventory level.
	   Example: 2022-12-06T15:13:50+08:00
	*/
	UpdatedAt string `json:"updated_at,omitempty" url:"-"`

	/*
	   The variant ID of this inventoryLevel.
	   Example: 18057039432358082693350282
	*/
	VariantId string `json:"variant_id,omitempty" url:"-"`

	/*
	   The available quantity of products owned by the location.
	   Example: 10
	*/
	Available int `json:"available,omitempty" url:"-"`

	/*
	   The quantity allocated to orders but not yet shipped.
	   Example: 10
	*/
	Committed int `json:"committed,omitempty" url:"-"`
}
